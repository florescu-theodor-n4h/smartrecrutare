...
public record ProfilCandidatRequest(
        @ArraySchema(schema = @Schema(example = "java"), minItems = 1, maxItems = 50)
        @Size(min = 1, max = 50) Set<@NotBlank @Size(max = 80) String> abilitati,

        @ArraySchema(schema = @Schema(example = "bucuresti"), maxItems = 20)
        @Size(max = 20) Set<@NotBlank @Size(max = 120) String> locatiiPreferate,

        @Schema(description = "Tipul de contract preferat", example = "Full-time", maxLength = 80)
        @Size(max = 80) String tipContractPreferat,

        @ArraySchema(schema = @Schema(example = "spring"), maxItems = 50)
        @Size(max = 50) Set<@NotBlank @Size(max = 80) String> cuvinteCheie
) {
}
